...
myBiomodCV <- bm_CrossValidation(bm.format = myBiomodData, nb.rep = 3) 
...
DataSplitTable <- bm_CrossValidation(ch_SpheCoop_data, 
                                    k = 5, 
                                    nb.rep = 3, 
                                    do.full.models = F)

DataSplitTable.y <- bm_CrossValidation(myBiomodData, 
                                       stratified.cv = T, 
                                       stratify = "y", 
                                       k = 2)  
...
ch_SpheCoop_models <- BIOMOD_Modeling(
                    ...
                    var.import = 2, #Number of permutation to estimate variable importance
                    do.full.models = F,
                    scale.models = T, #if true, models calibrated and evaluated with the whole dataset are done
                    modeling.id = "SpheCoop-models", #character, the ID (=name) of modeling procedure. A random number by default.
                    seed.val = 42
                        )
...
myCalibLines <- get_calib_lines(ch_SpheCoop_models)
plot(ch_SpheCoop_models, calib.lines = myCalibLines)
...
myEval <- get_evaluations(ch_SpheCoop_models) 
myEval$CV.strategy <- "block" 
myEval$CV.strategy[grepl("13", myEval$full.name)] <- "Full" 
myEval$CV.strategy[grepl("11|12", myEval$full.name)] <- "Block" 
head(myEval)
