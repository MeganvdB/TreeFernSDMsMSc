...
ch_SpheCoop_models <- BIOMOD_Modeling(
                        ...
                        do.full.models = F,
                        scale.models = TRUE,
                        modeling.id = "GFDL-ESM4-model_run_tuned",
                        seed.val = 42
                        )
...
# Assuming indiv_SpheCoop_scores is your data frame
write.csv(indiv_SpheCoop_scores, file = "tuned-Model-evaluations.csv", row.names = FALSE)

dim(indiv_SpheCoop_scores)
dimnames(indiv_SpheCoop_scores)
...
# Load models can be done like this for all models
